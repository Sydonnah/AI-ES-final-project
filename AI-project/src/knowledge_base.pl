...
menu:-new(M,dialog('COVID-19 Diagnosis System')),
    ...
    send(H2,append,button(covid_Statistics,message(@prolog,updatestats))),
...
save_rsymptom(RSymptoms):-
    ...
    redefine_system_predicate('regularsymptoms'(_)),
    retractall(regularsymptoms(_)),assertz(regularsymptoms(New)),
    nl,write('Symptom:'),write(RSymptoms),write(' was added to the database').

save_dsymptom(DSymptoms):-
    deltasymptoms(Old),append(Old,[DSymptoms],New),
    redefine_system_predicate('deltasymptoms'(_)),
    retractall(deltasymptoms(_)),assertz(deltasymptoms(New)),
    nl,write('Symptom:'),write(DSymptoms),write(' was added to the database').

save_osymptom(OSymptoms):-

    omicronsymptoms(Old),append(Old,[OSymptoms],New),
    redefine_system_predicate('omicronsymptoms'(_)),
    retractall(omicronsymptoms(_)),assert(deltasymptoms(New)),
    nl,write('Symptom:'),write(OSymptoms),write(' was added to the database').
...
view_ucondition:- underlying(omicron,Who),nl,write('Omicron Underlying Conditions:'),nl,nl,write(Who).
...
save_patient(Name,Age,Gender,Temperature,Dizzy,Faint,Blur,SysR,DiaR,Fever,Fatigue,Head,Sore,Diffb,Chest,Losm,Cough,Lot,Run,Muscle,Sneeze,Boc,Cancer,
    ...
    (Riskfactor=<28 ->Deltacount is 0, Omicroncount is 0,Mildcount is 1, Severecount is 0,Totalcount is 1, send(Lbl24,append,'Covid-19 Diagnosis: Patient may have the Regular Covid-19 variant, Patient should quarantine for 14 days');

   (Riskfactor>28,Riskfactor=<58) ->Deltacount is 1, Omicroncount is 0,Mildcount is 0, Severecount is 1,Totalcount is 1, send(Lbl24,append,'Covid-19 Diagnosis: Patient may have the Delta Covid-19 variant, Patient should visit the doctor immediately');

    (Riskfactor>58,Underlyingfactor >=1) ->Deltacount is 0, Omicroncount is 1, Mildcount is 0, Severecount is 1, Underlyingcount is 1, Totalcount is 1, send(Lbl24,append,'Covid-19 Diagnosis: Patient may have the Ominicron Covid-19 variant, Patient should visit the doctor immediately')),
...
updatestats(Deltacount,Omicroncount,Mildcount,Severecount,Underlyingcount,Totalcount):-
    ...
    NewTcount is Total+Totalcount,

    retractall(stats(_,_,_,_,_,_)),

    asserta(stats(NewDcount,NewOcount,NewMcount,NewScount,NewUcount,NewTcount)),
    new(CS,dialog('Covid-19 Statistics')),send(CS,append,new(label)),

    send(CS,append,new(Lblt,label)),send(Lblt,append,'Total number of cases:'),
    send(CS,append,new(Lblt1,label)),send(Lblt1,append,NewTcount),


    Mildstat is(NewMcount/NewTcount)*100,
    send(CS,append,new(Lbl25,label)),send(Lbl25,append,'Percentage of persons with Mild Symptoms:'),
    send(CS,append,new(Lbl26,label)),send(Lbl26,append,Mildstat),

    Severestat is(NewScount/NewTcount)*100,
    send(CS,append,new(Lbl27,label)),send(Lbl27,append,'Percentage of Persons with Severe Symptoms:'),
    send(CS,append,new(Lbl28,label)),send(Lbl28,append,Severestat),

    Deltastat is(NewDcount/NewTcount)*100,
    send(CS,append,new(Lbl29,label)),send(Lbl29,append,'Percentage of Persons with Delta Variant:'),
    send(CS,append,new(Lbl30,label)),send(Lbl30,append,Deltastat),

    Omicronstat is(NewOcount/NewTcount)*100,
    send(CS,append,new(Lbl31,label)),send(Lbl31,append,'Percentage of Persons with Omicron Variant:'),
    send(CS,append,new(Lbl32,label)),send(Lbl32,append,Omicronstat),

    OUnderlyingstat is(NewUcount/NewTcount)*100,
